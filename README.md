...
- Offline creative-level sharing through `PBZ1-XXXX-XXXX` level codes.
...
### Unreleased - Creative Level Share Codes

- Added offline level-code export/import for creative levels.
- Added `PBZ1-XXXX-XXXX` style share codes using uppercase letters, numbers, and hyphens.
- Added checksum validation so mistyped or incomplete codes are rejected.
- Added gameplay HUD `Share` support to copy the current level code to the clipboard.
- Added Creative Mode level-code import support from the level select UI.
- Added support for pasting a level code into the in-game custom editor's level ID field and loading it into the editable grid.
- Changed new creative-level auto IDs from `custom_001` style to shorter `C001` style.
- Added drag-paint support in the in-game custom editor grid.
